...
class FacialDataGenerator(tf.keras.utils.Sequence):
    ...
    def __init__(self, xml, batch_size=10, target_size=(224, 224), shuffle=True):
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.batch_size):
            ...
            edges = cv2.Canny(rotated, canny_low, canny_high)
            img_batch.append(edges)
...
model = tf.keras.models.Sequential([
    tf.keras.layers.Conv2D(16, (5, 5), activation='relu', input_shape=(224, 224, 1)),
...
train_gen = FacialDataGenerator("labels_ibug_300W_train.xml", target_size=(224, 224))
validation_gen = FacialDataGenerator("labels_ibug_300W_test.xml", target_size=(224, 224))
...
if False:
    ...
    img = cv2.Canny(cv2.imread(test_image), canny_low, canny_high)
    landmarks = [0] * 24
    wrapped = np.array([cv2.resize(img, (224, 224))])
